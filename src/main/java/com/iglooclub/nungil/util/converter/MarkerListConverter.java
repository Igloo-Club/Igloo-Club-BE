...
@Converter
public class MarkerListConverter implements AttributeConverter<List<Marker>, String> {
    private static final ObjectMapper mapper = new ObjectMapper()
            .configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false)
            .configure(DeserializationFeature.FAIL_ON_NULL_FOR_PRIMITIVES, false);

    @Override
    public String convertToDatabaseColumn(List<Marker> attribute) {
        try {
    ...
    @Override
    public List<Marker> convertToEntityAttribute(String dbData) {
        if (dbData == null || dbData.trim().isEmpty()) {
            return Collections.emptyList(); // dbData가 null이거나 빈 문자열일 경우 빈 리스트 반환
        }
        TypeReference<List<Marker>> typeReference = new TypeReference<List<Marker>>() {};
        try {
